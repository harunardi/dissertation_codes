...
#######################################################################################################
def main_unfold_2D_rect(group, N, I_max, J_max, TOT, SIGS_reshaped, BC, dx, dy, D, chi, NUFIS, precond, v, Beff, omega, l, dTOT, dSIGS_reshaped, dNUFIS, map_detector, map_zone, case_name, case_name_base, x, y, validity_INVERT, validity_ZONE, validity_SCAN, validity_BRUTE, validity_BACK, validity_GREEDY):
    output_dir = f'OUTPUTS/{case_name_base}/{case_name}_UNFOLDING'

    ########################################################################################################
    # Configure logging to write to both the console and a file
    os.makedirs(f'{output_dir}', exist_ok=True)
    log_filename = f"{output_dir}/output.log"
    logging.basicConfig(
        level=logging.INFO,  # Capture INFO level logs
        format="%(message)s",  # Only print messages, no timestamps or log levels
        handlers=[
            logging.FileHandler(log_filename, mode='w'),  # Write logs to file
            logging.StreamHandler(sys.stdout)  # Print logs to terminal
        ]
    )

    # Redirect `print()` to `logging.info` so everything is logged
    print = logging.info
...
##### Forward Simulation
    ...
    matrix_builder = MatrixBuilderForward2D(group, N, conv, TOT, SIGS_reshaped, BC, dx, dy, D, chi, NUFIS)
    M, F_FORWARD = matrix_builder.build_forward_matrices()

    solver = SolverFactory.get_solver_power2D(solver_type, group, N, conv, M, F_FORWARD, dx, dy, precond, tol=1E-10)
    keff, PHI_temp = solver.solve()
    PHI, PHI_reshaped, PHI_reshaped_plot = PostProcessor.postprocess_power2D(PHI_temp, conv, group, N, I_max, J_max)
...
##### Adjoint Simulation
    ...
    matrix_builder = MatrixBuilderAdjoint2D(group, N, conv, TOT, SIGS_reshaped, BC, dx, dy, D, chi, NUFIS)
    M, F_ADJOINT = matrix_builder.build_adjoint_matrices()

    solver = SolverFactory.get_solver_power2D(solver_type, group, N, conv, M, F_ADJOINT, dx, dy, precond, tol=1E-10)
    keff, PHI_ADJ_temp = solver.solve()
    PHI_ADJ, PHI_ADJ_reshaped, PHI_ADJ_reshaped_plot = PostProcessor.postprocess_power2D(PHI_ADJ_temp, conv, group, N, I_max, J_max)
...
##### Noise Simulation
    ...
    matrix_builder = MatrixBuilderNoise2D(group, N, conv, TOT, SIGS_reshaped, BC, dx, dy, D, chi, NUFIS, keff, v, Beff, omega, l, dTOT, dSIGS_reshaped, dNUFIS)
    M, dS = matrix_builder.build_noise_matrices()

    solver = SolverFactory.get_solver_fixed2D(solver_type, group, N, conv, M, dS, PHI, dx, dy, precond, tol=1e-10)

    dPHI_temp = solver.solve()
    dPHI, dPHI_reshaped, dPHI_reshaped_plot = PostProcessor.postprocess_fixed2D(dPHI_temp, conv, group, N, I_max, J_max)
...
##### 02. Solve
    ...
    for g in range(group):
        Utils.plot_2D_fixed_general(solver_type, dPHI_SOLVE_reshaped_plot[g], x, y, g+1, cmap='viridis', output=output_SOLVE, varname=f'dPHI_full', case_name=case_name, title=f'2D Plot of dPHI{g+1} (Reference) Magnitude', process_data='magnitude')
        Utils.plot_2D_fixed_general(solver_type, dPHI_SOLVE_reshaped_plot[g], x, y, g+1, cmap='viridis', output=output_SOLVE, varname=f'dPHI_full', case_name=case_name, title=f'2D Plot of dPHI{g+1} (Reference) Phase', process_data='phase')
        Utils.plot_2D_fixed_general(solver_type, S_all_reshaped_plot[g], x, y, g+1, cmap='viridis', output=output_SOLVE, varname=f'S_full', case_name=case_name, title=f'2D Plot of S{g+1} (Reference) Magnitude', process_data='magnitude')
        Utils.plot_2D_fixed_general(solver_type, S_all_reshaped_plot[g], x, y, g+1, cmap='viridis', output=output_SOLVE, varname=f'S_full', case_name=case_name, title=f'2D Plot of S{g+1} (Reference) Phase', process_data='phase')
    ...
    for g in range(group):
        Utils.plot_2D_fixed_general(solver_type, dS_SOLVE_reshaped_plot[g], x, y, g+1, cmap='viridis', output=output_SOLVE, varname=f'dS_SOLVE', case_name=case_name, title=f'2D Plot of dS{g+1}_SOLVE Magnitude', process_data='magnitude')
        Utils.plot_2D_fixed_general(solver_type, dS_SOLVE_reshaped_plot[g], x, y, g+1, cmap='viridis', output=output_SOLVE, varname=f'dS_SOLVE', case_name=case_name, title=f'2D Plot of dS{g+1}_SOLVE Phase', process_data='phase')
    ...
    for g in range(group):
        Utils.plot_2D_fixed_general(solver_type, diff_S_reshaped[g], x, y, g+1, cmap='viridis', output=output_SOLVE, varname=f'diff_dS', case_name=case_name, title=f'2D Plot of Difference dS{g+1}_SOLVE Magnitude', process_data='magnitude')
        Utils.plot_2D_fixed_general(solver_type, diff_S_reshaped[g], x, y, g+1, cmap='viridis', output=output_SOLVE, varname=f'diff_dS', case_name=case_name, title=f'2D Plot of Difference dS{g+1}_SOLVE Phase', process_data='phase')
...
##### 03. INVERT
    ...
    Utils.plot_2D_fixed_general(solver_type, map_detector_reshaped, x, y, g+1, cmap='viridis', output=output_INVERT, varname=f'map_detector_reshaped', case_name=case_name, title=f'2D Plot of Detector Map', process_data='magnitude')
    ...
    for g in range(group):
        Utils.plot_2D_fixed_general(solver_type, dPHI_meas_reshaped[g], x, y, g+1, cmap='viridis', output=output_INVERT, varname=f'dPHI_meas', case_name=case_name, title=f'2D Plot of dPHI{g+1} Measured Magnitude', process_data='magnitude')
        Utils.plot_2D_fixed_general(solver_type, dPHI_meas_reshaped[g], x, y, g+1, cmap='viridis', output=output_INVERT, varname=f'dPHI_meas', case_name=case_name, title=f'2D Plot of dPHI{g+1} Measured Phase', process_data='phase')
    ...
    # Plot dPHI_interp
    for g in range(group):
        Utils.plot_2D_fixed_general(solver_type, dPHI_interp_array[g], x, y, g+1, cmap='viridis', output=output_INVERT, varname=f'dPHI_interp', case_name=case_name, title=f'2D Plot of dPHI{g+1} Interpolated Magnitude', process_data='magnitude')
        Utils.plot_2D_fixed_general(solver_type, dPHI_interp_array[g], x, y, g+1, cmap='viridis', output=output_INVERT, varname=f'dPHI_interp', case_name=case_name, title=f'2D Plot of dPHI{g+1} Interpolated Phase', process_data='phase')
    ...
    for g in range(group):
        Utils.plot_2D_fixed_general(solver_type, diff_dPHI_interp_reshaped[g], x, y, g+1, cmap='viridis', output=output_INVERT, varname=f'diff_dPHI_interp', case_name=case_name, title=f'2D Plot of dPHI{g+1} Diff Full vs Interpolated Magnitude', process_data='magnitude')
        Utils.plot_2D_fixed_general(solver_type, diff_dPHI_interp_reshaped[g], x, y, g+1, cmap='viridis', output=output_INVERT, varname=f'diff_dPHI_interp', case_name=case_name, title=f'2D Plot of dPHI{g+1} Diff Full vs Interpolated Phase', process_data='phase')
    ...
    for g in range(group):
        Utils.plot_2D_fixed_general(solver_type, dS_unfold_INVERT_reshaped_plot[g], x, y, g+1, cmap='viridis', output=output_INVERT, varname=f'dS_unfold_INVERT', case_name=case_name, title=f'2D Plot of dS{g+1}_INVERT Magnitude', process_data='magnitude')
        Utils.plot_2D_fixed_general(solver_type, dS_unfold_INVERT_reshaped_plot[g], x, y, g+1, cmap='viridis', output=output_INVERT, varname=f'dS_unfold_INVERT', case_name=case_name, title=f'2D Plot of dS{g+1}_INVERT Phase', process_data='phase')
    ...
    for g in range(group):
        Utils.plot_2D_fixed_general(solver_type, diff_S_INVERT_reshaped[g], x, y, g+1, cmap='viridis', output=output_INVERT, varname=f'diff_dS_unfold_INVERT', case_name=case_name, title=f'2D Plot of dS{g+1}_INVERT Difference Magnitude', process_data='magnitude')
        Utils.plot_2D_fixed_general(solver_type, diff_S_INVERT_reshaped[g], x, y, g+1, cmap='viridis', output=output_INVERT, varname=f'diff_dS_unfold_INVERT', case_name=case_name, title=f'2D Plot of dS{g+1}_INVERT Difference Phase', process_data='phase')
...
##### 04. ZONE
    ...
    Utils.plot_2D_fixed_general(solver_type, map_zone_plot_reshaped, x, y, g+1, cmap='viridis', output=output_ZONE, varname=f'map_zone', case_name=case_name, title=f'2D Plot of Zone Map', process_data='magnitude')
    ...
    for g in range(group):
        Utils.plot_2D_fixed_general(solver_type, dS_unfold_ZONE_reshaped_plot[g], x, y, g+1, cmap='viridis', output=output_ZONE, varname=f'dS_unfold_ZONE', case_name=case_name, title=f'2D Plot of dS{g+1}_ZONE Magnitude', process_data='magnitude')
        Utils.plot_2D_fixed_general(solver_type, dS_unfold_ZONE_reshaped_plot[g], x, y, g+1, cmap='viridis', output=output_ZONE, varname=f'dS_unfold_ZONE', case_name=case_name, title=f'2D Plot of dS{g+1}_ZONE Phase', process_data='phase')
    ...
    for g in range(group):
        Utils.plot_2D_fixed_general(solver_type, diff_S_ZONE_reshaped[g], x, y, g+1, cmap='viridis', output=output_ZONE, varname=f'diff_dS_unfold_ZONE', case_name=case_name, title=f'2D Plot of dS{g+1}_ZONE Difference Magnitude', process_data='magnitude')
        Utils.plot_2D_fixed_general(solver_type, diff_S_ZONE_reshaped[g], x, y, g+1, cmap='viridis', output=output_ZONE, varname=f'diff_dS_unfold_ZONE', case_name=case_name, title=f'2D Plot of dS{g+1}_ZONE Difference Phase', process_data='phase')
...
##### 05. SCAN
    ...
    for g in range(group):
        Utils.plot_2D_fixed_general(solver_type, delta_all_full_plot[g], x, y, g+1, cmap='viridis', output=output_SCAN, varname=f'delta_AB{g+1}', case_name=case_name, title=f'2D Plot of delta_AB{g+1} Magnitude', process_data='magnitude')
    ...
    for g in range(group):
        Utils.plot_2D_fixed_general(solver_type, delta_all_full_inv_plot[g], x, y, g+1, cmap='viridis', output=output_SCAN, varname=f'delta_AB{g+1}_INV', case_name=case_name, title=f'2D Plot of delta_AB{g+1}_INV Magnitude', process_data='magnitude')
    ...
    for g in range(group):
        Utils.plot_2D_fixed_general(solver_type, dS_unfold_SCAN_reshaped_plot[g], x, y, g+1, cmap='viridis', output=output_SCAN, varname=f'dS_unfold_SCAN', case_name=case_name, title=f'2D Plot of dS{g+1}_SCAN Magnitude', process_data='magnitude')
        Utils.plot_2D_fixed_general(solver_type, dS_unfold_SCAN_reshaped_plot[g], x, y, g+1, cmap='viridis', output=output_SCAN, varname=f'dS_unfold_SCAN', case_name=case_name, title=f'2D Plot of dS{g+1}_SCAN Phase', process_data='phase')
    ...
    for g in range(group):
        Utils.plot_2D_fixed_general(solver_type, diff_S_SCAN_reshaped[g], x, y, g+1, cmap='viridis', output=output_SCAN, varname=f'diff_dS_unfold_SCAN', case_name=case_name, title=f'2D Plot of dS{g+1}_SCAN Difference Magnitude', process_data='magnitude')
        Utils.plot_2D_fixed_general(solver_type, diff_S_SCAN_reshaped[g], x, y, g+1, cmap='viridis', output=output_SCAN, varname=f'diff_dS_unfold_SCAN', case_name=case_name, title=f'2D Plot of dS{g+1}_SCAN Difference Phase', process_data='phase')
...
##### 06. BRUTE FORCE
    ...
    for g in range(group):
        Utils.plot_2D_fixed_general(solver_type, dPHI_meas_reshaped[g], x, y, g+1, cmap='viridis', output=output_BRUTE, varname=f'dPHI_meas', case_name=case_name, title=f'2D Plot of dPHI{g+1} Measured Magnitude', process_data='magnitude')
        Utils.plot_2D_fixed_general(solver_type, dPHI_meas_reshaped[g], x, y, g+1, cmap='viridis', output=output_BRUTE, varname=f'dPHI_meas', case_name=case_name, title=f'2D Plot of dPHI{g+1} Measured Phase', process_data='phase')
    ...
    if valid_solution_BRUTE:
        ...
        for g in range(group):
            Utils.plot_2D_fixed_general(solver_type, dPHI_BRUTE_reshaped[g], x, y, g+1, cmap='viridis', output=output_BRUTE, varname=f'dPHI_BRUTE', case_name=case_name, title=f'2D Plot of dPHI{g+1} BRUTE Magnitude', process_data='magnitude')
            Utils.plot_2D_fixed_general(solver_type, dPHI_BRUTE_reshaped[g], x, y, g+1, cmap='viridis', output=output_BRUTE, varname=f'dPHI_BRUTE', case_name=case_name, title=f'2D Plot of dPHI{g+1} BRUTE Phase', process_data='phase')
        ...
        for g in range(group):
            Utils.plot_2D_fixed_general(solver_type, dS_unfold_BRUTE_plot[g], x, y, g+1, cmap='viridis', output=output_BRUTE, varname=f'dS_BRUTE', case_name=case_name, title=f'2D Plot of dS{g+1} BRUTE Magnitude', process_data='magnitude')
            Utils.plot_2D_fixed_general(solver_type, dS_unfold_BRUTE_plot[g], x, y, g+1, cmap='viridis', output=output_BRUTE, varname=f'dS_BRUTE', case_name=case_name, title=f'2D Plot of dS{g+1} BRUTE Phase', process_data='phase')
        ...
        for g in range(group):
            Utils.plot_2D_fixed_general(solver_type, diff_S_BRUTE_reshaped[g], x, y, g+1, cmap='viridis', output=output_BRUTE, varname=f'diff_dS', case_name=case_name, title=f'2D Plot of dS{g+1} BRUTE Difference Magnitude', process_data='magnitude')
            Utils.plot_2D_fixed_general(solver_type, diff_S_BRUTE_reshaped[g], x, y, g+1, cmap='viridis', output=output_BRUTE, varname=f'diff_dS', case_name=case_name, title=f'2D Plot of dS{g+1} BRUTE Difference Phase', process_data='phase')
...
##### 07. BACKWARD ELIMINATION
    ...
    if valid_solution_BACK:
        ...
        for g in range(group):
            Utils.plot_2D_fixed_general(solver_type, dPHI_BACK_reshaped[g], x, y, g+1, cmap='viridis', output=output_BACK, varname=f'dPHI_BACK', case_name=case_name, title=f'2D Plot of dPHI{g+1} BACK Magnitude', process_data='magnitude')
            Utils.plot_2D_fixed_general(solver_type, dPHI_BACK_reshaped[g], x, y, g+1, cmap='viridis', output=output_BACK, varname=f'dPHI_BACK', case_name=case_name, title=f'2D Plot of dPHI{g+1} BACK Phase', process_data='phase')
        ...
        for g in range(group):
            Utils.plot_2D_fixed_general(solver_type, dS_unfold_BACK_plot[g], x, y, g+1, cmap='viridis', output=output_BACK, varname=f'dS_BACK', case_name=case_name, title=f'2D Plot of dS{g+1} BACK Magnitude', process_data='magnitude')
            Utils.plot_2D_fixed_general(solver_type, dS_unfold_BACK_plot[g], x, y, g+1, cmap='viridis', output=output_BACK, varname=f'dS_BACK', case_name=case_name, title=f'2D Plot of dS{g+1} BACK Phase', process_data='phase')
        ...
        for g in range(group):
            Utils.plot_2D_fixed_general(solver_type, diff_S_BACK_reshaped[g], x, y, g+1, cmap='viridis', output=output_BACK, varname=f'diff_dS_BACK', case_name=case_name, title=f'2D Plot of dS{g+1} BACK Difference Magnitude', process_data='magnitude')
            Utils.plot_2D_fixed_general(solver_type, diff_S_BACK_reshaped[g], x, y, g+1, cmap='viridis', output=output_BACK, varname=f'diff_dS_BACK', case_name=case_name, title=f'2D Plot of dS{g+1} BACK Difference Phase', process_data='phase')

    else:
        print("No valid solution found with backward elimination.")

    if np.allclose(S, dS_unfold_BACK_temp, atol=1E-06):
...
##### 08. GREEDY
    ...
    if valid_solution_GREEDY:
        ...
        for g in range(group):
            Utils.plot_2D_fixed_general(solver_type, dPHI_GREEDY_reshaped[g], x, y, g+1, cmap='viridis', output=output_GREEDY, varname=f'dPHI_GREEDY', case_name=case_name, title=f'2D Plot of dPHI{g+1} GREEDY Magnitude', process_data='magnitude')
            Utils.plot_2D_fixed_general(solver_type, dPHI_GREEDY_reshaped[g], x, y, g+1, cmap='viridis', output=output_GREEDY, varname=f'dPHI_GREEDY', case_name=case_name, title=f'2D Plot of dPHI{g+1} GREEDY Phase', process_data='phase')
        ...
        for g in range(group):
            Utils.plot_2D_fixed_general(solver_type, dS_unfold_GREEDY_plot[g], x, y, g+1, cmap='viridis', output=output_GREEDY, varname=f'dS_GREEDY', case_name=case_name, title=f'2D Plot of dS{g+1} GREEDY Magnitude', process_data='magnitude')
            Utils.plot_2D_fixed_general(solver_type, dS_unfold_GREEDY_plot[g], x, y, g+1, cmap='viridis', output=output_GREEDY, varname=f'dS_GREEDY', case_name=case_name, title=f'2D Plot of dS{g+1} GREEDY Phase', process_data='phase')
        ...
        for g in range(group):
            Utils.plot_2D_fixed_general(solver_type, diff_S_GREEDY_reshaped[g], x, y, g+1, cmap='viridis', output=output_GREEDY, varname=f'diff_dS_GREEDY', case_name=case_name, title=f'2D Plot of dS{g+1} GREEDY Difference Magnitude', process_data='magnitude')
            Utils.plot_2D_fixed_general(solver_type, diff_S_GREEDY_reshaped[g], x, y, g+1, cmap='viridis', output=output_GREEDY, varname=f'diff_dS_GREEDY', case_name=case_name, title=f'2D Plot of dS{g+1} GREEDY Difference Phase', process_data='phase')
    ...
    return validity_INVERT, validity_ZONE, validity_SCAN, validity_BRUTE, validity_BACK, validity_GREEDY
