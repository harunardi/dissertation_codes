...
##### 01. Green's Function Generation
    ...
    for g in range(group):
        for n in range(N):
            ...
            with open(f'{output_dir}/{case_name}_UNFOLDING/{case_name}_01_GENERATE/Green_g{g+1}_n{n+1}.json', 'w') as json_file:
                json.dump(output, json_file, indent=4)
...
##### 02. Solve
    ...
    for g in range(group):
        ...
        plt.legend()
        plt.grid(True)
        plt.savefig(f'{output_dir}/{case_name}_UNFOLDING/{case_name}_02_SOLVE/{case_name}_02_SOLVE_abs_G{g+1}.png')
        plt.clf()
        ...
        plt.legend()
        plt.grid(True)
        plt.savefig(f'{output_dir}/{case_name}_UNFOLDING/{case_name}_02_SOLVE/{case_name}_02_SOLVE_phase_G{g+1}.png')
        plt.clf()
        ...
        plt.legend()
        plt.grid(True)
        plt.savefig(f'{output_dir}/{case_name}_UNFOLDING/{case_name}_02_SOLVE/{case_name}_02_SOLVE_real_G{g+1}.png')
        plt.clf()
        ...
        plt.legend()
        plt.grid(True)
        plt.savefig(f'{output_dir}/{case_name}_UNFOLDING/{case_name}_02_SOLVE/{case_name}_02_SOLVE_dS_G{g+1}.png')
    # OUTPUT
    output = {}
    for g in range(group):
        dPHI_groupname = f'dPHI{g+1}'
    ...
    # Save data to JSON file
    with open(f'{output_dir}/{case_name}_UNFOLDING/{case_name}_02_SOLVE/{case_name}_02_SOLVE_output.json', 'w') as json_file:
        json.dump(output, json_file, indent=4)
    ...
    # Save data to JSON file
    with open(f'{output_dir}/{case_name}_UNFOLDING/{case_name}_02_SOLVE/{case_name}_02_SOLVE_dS_output.json', 'w') as json_file:
        json.dump(output, json_file, indent=4)
##### 03. INVERT
    ...
    for g in range(group):
        ...
        plt.grid()
        plt.savefig(f'{output_dir}/{case_name}_UNFOLDING/{case_name}_03_INVERT/{case_name}_dPHI_meas_magnitude_G{g+1}.png')
    for g in range(group):
        plt.figure()
        ...
        plt.xlabel('Distance from core centre [cm]')
        plt.grid()
        plt.savefig(f'{output_dir}/{case_name}_UNFOLDING/{case_name}_03_INVERT/{case_name}_dPHI_meas_phase_G{g+1}.png')
    # --------------- INTERPOLATE dPHI -------------------
    ...
    for g in range(group):
        ...
        plt.xlabel('Distance from core centre [cm]')
        plt.grid()
        plt.savefig(f'{output_dir}/{case_name}_UNFOLDING/{case_name}_03_INVERT/{case_name}_dPHI_interp_magnitude_G{g+1}.png')
    for g in range(group):
        plt.figure()
        plt.plot(x, np.degrees(np.angle(dPHI_reshaped[g])), 'g-', label=f'Group {g+1} - dPHI_sol')
        ...
        plt.xlabel('Distance from core centre [cm]')
        plt.grid()
        plt.savefig(f'{output_dir}/{case_name}_UNFOLDING/{case_name}_03_INVERT/{case_name}_dPHI_interp_phase_G{g+1}.png')
    ...
    for g in range(group):
        ...
        plt.xlabel('Distance from core centre [cm]')
        plt.grid()
        plt.savefig(f'{output_dir}/{case_name}_UNFOLDING/{case_name}_03_INVERT/{case_name}_dS_INVERT_magnitude_G{g+1}.png')
    # OUTPUT
    print(f'Generating JSON output for dS')
    output_direct1 = {}
    for g in range(group):
    ...
    # Save data to JSON file
    with open(f'{output_dir}/{case_name}_UNFOLDING/{case_name}_03_INVERT/{case_name}_dS_INVERT_output.json', 'w') as json_file:
...
##### 05. SCAN
    ...
    for g in range(group):
        ...
        plt.xlabel('Distance from core centre [cm]')
        plt.grid()
        plt.savefig(f'{output_dir}/{case_name}_UNFOLDING/{case_name}_05_SCAN/{case_name}_delta_all_G{g+1}.png')
    # Flatten dPHI_sol_temp_groups to a 1D list
    ...
    for g in range(group):
        ...
        plt.xlabel('Distance from core centre [cm]')
        plt.grid()
        plt.savefig(f'{output_dir}/{case_name}_UNFOLDING/{case_name}_05_SCAN/{case_name}_delta_all_inv_G{g+1}.png')
    ...
    M = np.abs(dPHI_meas_new/G_sol_mat_new)
    ...
    for g in range(group):
        ...
        plt.legend()
        plt.grid(True)
        plt.savefig(f'{output_dir}/{case_name}_UNFOLDING/{case_name}_05_SCAN/{case_name}_dS_G{g+1}.png')
    # OUTPUT
    print(f'Generating JSON output for dS')
    output = {}
    for g in range(group):
        ...
        output[dS_unfold_groupname] = dS_unfold_list
    # Save data to JSON file
    with open(f'{output_dir}/{case_name}_UNFOLDING/{case_name}_05_SCAN/{case_name}_dS_output.json', 'w') as json_file:
        json.dump(output, json_file, indent=4)
...
##### 06. BRUTE FORCE
    ...
    for num_source in range(1, num_atoms + 1):
        ...
        for subset in combinations(atom_keys, num_source):
            if iter_BRUTE % (20 * num_atoms) == 0:
                print(f"Iteration = {iter_BRUTE}, subset progress = {(iter/len(list(combinations(atom_keys, num_source)))*100):.2f}%")
    ...
    if valid_solution_BRUTE:
        ...
        for g in range(group):
            ...
            plt.xlabel('Distance from core centre [cm]')
            plt.grid()
            plt.savefig(f'{output_dir}/{case_name}_UNFOLDING/{case_name}_06_BRUTE/{case_name}_dPHI_BRUTE_magnitude_G{g+1}.png')
        ...
        for g in range(group):
            ...
            plt.xlabel('Distance from core centre [cm]')
            plt.grid()
            plt.savefig(f'{output_dir}/{case_name}_UNFOLDING/{case_name}_06_BRUTE/{case_name}_dPHI_BRUTE_phase_G{g+1}.png')
        ...
        for g in range(group):
            ...
            plt.xlabel('Distance from core centre [cm]')
            plt.grid()
            plt.savefig(f'{output_dir}/{case_name}_UNFOLDING/{case_name}_06_BRUTE/{case_name}_dPHI_BRUTE_real_G{g+1}.png')
        ...
        for g in range(group):
            ...
            plt.xlabel('Distance from core centre [cm]')
            plt.grid()
            plt.savefig(f'{output_dir}/{case_name}_UNFOLDING/{case_name}_06_BRUTE/{case_name}_dS_BRUTE_magnitude_G{g+1}.png')
...
##### 07. BACKWARD ELIMINATION
    ...
    if valid_solution_BACK:
        ...
        for g in range(group):
            ...
            plt.xlabel('Distance from core centre [cm]')
            plt.grid()
            plt.savefig(f'{output_dir}/{case_name}_UNFOLDING/{case_name}_07_BACK/{case_name}_dPHI_BACK_magnitude_G{g+1}.png')
        ...
        for g in range(group):
            ...
            plt.xlabel('Distance from core centre [cm]')
            plt.grid()
            plt.savefig(f'{output_dir}/{case_name}_UNFOLDING/{case_name}_07_BACK/{case_name}_dPHI_BACK_phase_G{g+1}.png')
        ...
        for g in range(group):
            ...
            plt.xlabel('Distance from core centre [cm]')
            plt.grid()
            plt.savefig(f'{output_dir}/{case_name}_UNFOLDING/{case_name}_07_BACK/{case_name}_dPHI_BACK_real_G{g+1}.png')
        ...
        for g in range(group):
            ...
            plt.xlabel('Distance from core centre [cm]')
            plt.grid()
            plt.savefig(f'{output_dir}/{case_name}_UNFOLDING/{case_name}_07_BACK/{case_name}_dS_BACK_magnitude_G{g+1}.png')
...
##### 08. GREEDY
    ...
    for g in range(group):
        ...
        plt.xlabel('Distance from core centre [cm]')
        plt.grid()
        plt.savefig(f'{output_dir}/{case_name}_UNFOLDING/{case_name}_08_GREEDY/{case_name}_G_matrix_score_G{g+1}.png')
    ...
    if valid_solution_GREEDY:
        ...
        for g in range(group):
            ...
            plt.xlabel('Distance from core centre [cm]')
            plt.grid()
            plt.savefig(f'{output_dir}/{case_name}_UNFOLDING/{case_name}_08_GREEDY/{case_name}_dPHI_GREEDY_magnitude_G{g+1}.png')
        ...
        for g in range(group):
            ...
            plt.xlabel('Distance from core centre [cm]')
            plt.grid()
            plt.savefig(f'{output_dir}/{case_name}_UNFOLDING/{case_name}_08_GREEDY/{case_name}_dPHI_GREEDY_phase_G{g+1}.png')
        ...
        for g in range(group):
            ...
            plt.xlabel('Distance from core centre [cm]')
            plt.grid()
            plt.savefig(f'{output_dir}/{case_name}_UNFOLDING/{case_name}_08_GREEDY/{case_name}_dPHI_GREEDY_real_G{g+1}.png')
        ...
        for g in range(group):
            ...
            plt.xlabel('Distance from core centre [cm]')
            plt.grid()
            plt.savefig(f'{output_dir}/{case_name}_UNFOLDING/{case_name}_08_GREEDY/{case_name}_dS_GREEDY_magnitude_G{g+1}.png')
