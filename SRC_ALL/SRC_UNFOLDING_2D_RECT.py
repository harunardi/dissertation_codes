...
#######################################################################################################
def main_unfold_2D_rect_base(group, N, I_max, J_max, TOT, SIGS_reshaped, BC, dx, dy, D, chi, NUFIS, precond, v, Beff, omega, l, dTOT, dSIGS_reshaped, dNUFIS, map_detector, case_name, case_name_base, case_name2, x, y):
...
#######################################################################################################
def main_unfold_2D_rect_old_methods(dPHI_temp_meas, dPHI_temp, S, G_matrix, group, N, I_max, J_max, conv, map_detector, map_zone, case_name, case_name_base, x, y, validity_INVERT, validity_ZONE, validity_SCAN):
    output_dir = f'../OUTPUTS/{case_name_base}/{case_name}_UNFOLDING'
    max_conv = max(conv)
    conv_array = np.array(conv)
...
#######################################################################################################
def main_unfold_2D_rect_brute(dPHI_temp_meas, dPHI_temp, S, G_matrix, group, N, I_max, J_max, conv, map_detector, case_name, case_name_base, x, y, validity_BRUTE):
    output_dir = f'../OUTPUTS/{case_name_base}/{case_name}_UNFOLDING'
    max_conv = max(conv)
    conv_array = np.array(conv)
    non_zero_conv = np.nonzero(conv)[0]
...
#######################################################################################################
def main_unfold_2D_rect_back(dPHI_temp_meas, dPHI_temp, S, G_matrix, group, N, I_max, J_max, conv, case_name, case_name_base, x, y, validity_BACK):
    output_dir = f'../OUTPUTS/{case_name_base}/{case_name}_UNFOLDING'
    max_conv = max(conv)
    conv_array = np.array(conv)
    non_zero_conv = np.nonzero(conv)[0]
...
#######################################################################################################
def main_unfold_2D_rect_greedy(dPHI_temp_meas, dPHI_temp, S, G_matrix, group, N, I_max, J_max, conv, case_name, case_name_base, x, y, validity_GREEDY):
    output_dir = f'../OUTPUTS/{case_name_base}/{case_name}_UNFOLDING'
    max_conv = max(conv)
    conv_array = np.array(conv)
    non_zero_conv = np.nonzero(conv)[0]
